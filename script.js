...
installBtn.addEventListener("click", async () => {
  if (deferredInstallPrompt) {
    ...
    showStatus("📲 تم طلب تثبيت التطبيق.");
    return;
  }

  // إذا لم يعطِ المتصفح نافذة التثبيت، نعطي المستخدم الطريقة اليدوية.
  if (window.matchMedia("(display-mode: standalone)").matches) {
    showStatus("التطبيق مثبت ومفتوح كتطبيق.");
  } else {
    alert(
      "باش تثبت الموقع كتطبيق:\n\n" +
      "1) افتح الموقع من Chrome عبر HTTPS.\n" +
      "2) ضغط على ⋮ (القائمة).\n" +
      "3) اختار «إضافة إلى الشاشة الرئيسية» أو «تثبيت التطبيق».\n\n" +
      "ملاحظة: فتح index.html مباشرة من مدير الملفات (file://) ما كيخليش Chrome يثبت PWA."
    );
  }
});
